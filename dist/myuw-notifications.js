var MyUWNotifications = (function (exports) {
  'use strict';
    ...
    _createClass(MyUWNotifications, [{
      ...
      */
      ...
      value: function connectedCallback() {
        ...
        this['limit'] = this.getAttribute('limit') || 999; // Element variables
        ...
        this.$notificationsCount = this.$notificationIds.length; // Display "see all" only if URL attribute was provided
        ...
        document.addEventListener('myuw-notification-dismissed', function (event) {
          // Process data passed with event
          if (event.detail.notificationId) {
            // Remove notification
            var index = _this2.$notificationIds.indexOf(event.detail.notificationId);

            _this2.$notificationIds.splice(index, 1); // Update for empty state


            if (_this2.$notificationIds.length <= 0) {
              _this2.$emptyState.classList.remove('hidden');
            }
          }
        }, false);
        /*
        ...
        */
        ...
        window.addEventListener('click', function (e) {
          if (_this2.$list.classList.contains('visible')) {
            ...
            _this2.$list.setAttribute('tabindex', '-1');
          }
        });
        /*
        ...
        */
        ...
        this.$bell.addEventListener('click', function (e) {
          ...
          if (_this2.$list.classList.contains('visible')) {
            ...
            _this2.$list.removeAttribute('tabindex');
      ...
      value: function componentReady(notifications) {
        ...
        if (_typeof(notifications) === 'object' && notifications.length > 0) {
          ...
          var limit = this['limit']; // Console log wrapper so debug message is safely posted 
          // if the browser doesn't have a console

          var log = Function.prototype.bind.call(console.debug, console); // create html structure for each notification
  ...
  var MyUWNotification =
  /*#__PURE__*/
  function (_HTMLElement) {
    _inherits(MyUWNotification, _HTMLElement);

    function MyUWNotification() {
      var _this;

      _classCallCheck(this, MyUWNotification);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(MyUWNotification).call(this));
      ...
      _this.shadowRoot.appendChild(MyUWNotification.template.content.cloneNode(true));

      return _this;
    }

    _createClass(MyUWNotification, [{
      key: "attributeChangedCallback",
      ...
      value: function dismissNotification(id) {
        ...
        this.parentNode.removeChild(this); // Dispatch custom event notifying client that a notification was dismissed
  ...
  exports.MyUWNotification = MyUWNotification;
  exports.MyUWNotifications = MyUWNotifications;
